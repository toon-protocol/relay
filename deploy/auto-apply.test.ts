 ...
 * box checkout, with `docker` stubbed on PATH so the box never needs a live
 * daemon to prove what THIS script does with what it answers.
 *
 ...
 * do", and the box sat on the new commit with the OLD containers, reporting
 * success forever. TOON_Network#164 ports the same fix to this bundle:
 ...
 * fixed.
 *
 * This bundle has no render.sh: `connector.toml` is committed whole, and
 * `docker-compose.yml` interpolates `${VAR:?msg}` straight out of
 * `deploy/.env` itself, the way `EDGE_HOST`/`READ_HOST`/`ACME_EMAIL`/
 * `RELAY_NOSTR_SECRET_KEY` already do. So a newly-required variable shows up
 * as a `docker compose pull`/`up -d` failure rather than a separate render
 * step, and the fixture below simulates exactly that one case in the stub
 * rather than reimplementing docker compose's whole interpolation engine.
 *
 * The first test is the fixture the issue asks for: a compose interpolation
 * that fails once after a fast-forward (a newly-required .env variable, the
 * same shape as TOON_Network#152's own regression), retried and reported
 * loudly by the very next run even though that run's fetch brings back
 * nothing new, and finally applied once the variable is added. The second
 * covers the box that has never written .applied at all.
 */
import {
  chmodSync,
  cpSync,
...
const ENV: Record<string, string> = {
  EDGE_HOST: 'proxy.relay.fixture.example',
  READ_HOST: 'relay-ws.fixture.example',
  ACME_EMAIL: 'you@fixture.example',
...
function freshOrigin(): { dir: string; sha: string } {
  ...
  chmodSync(join(deploy, 'auto-apply.sh'), 0o755);
...
function cloneBox(originDir: string): string {
  const dir = tmp('relay-box-');
  git(['clone', '--quiet', originDir, dir], undefined);
  return dir;
}

/** A commit that adds a newly-required .env variable to docker-compose.yml
 * -- the exact shape of the regression TOON_Network#152 found: a
 * fast-forward whose apply needs something the box's own .env does not have.
 * This bundle has no render.sh, so the new requirement is expressed the same
 * way the real, already-required variables are: `${VAR:?msg}` interpolated
 * by `docker compose` itself, straight out of `deploy/.env`. */
function addFixtureRequiredVar(originDir: string): string {
  const path = join(originDir, 'deploy', 'docker-compose.yml');
  const before = readFileSync(path, 'utf8');
  const after = `${before}
# Appended by deploy/auto-apply.test.ts (TOON_Network#164 test fixture).
...
// ── The stubbed world: docker on PATH ───────────────────────────────────────
// Every call auto-apply.sh itself makes: pull, up -d, ps -q, logs, and
// inspect for the health wait. It also simulates the ONE compose-interpolation
...
if [ "\${1:-}" = compose ]; then
  ...
  if grep -q 'RELAY_FIXTURE_FLAG:?' docker-compose.yml 2>/dev/null; then
    ...
    if [ -z "$flag" ]; then
      msg=$(sed -n 's/.*RELAY_FIXTURE_FLAG:?\\([^}]*\\)}.*/\\1/p' docker-compose.yml | head -n1)
      echo "error while interpolating docker-compose.yml: \\"RELAY_FIXTURE_FLAG\\": $msg" >&2
      exit 1
    fi
  fi
  rest="$*"
...
describe('a compose apply that fails once after a fast-forward (TOON_Network#164, porting TOON_Network#160)', () => {
  it('is retried and reported on every run -- even one whose fetch brings back nothing new -- and applies once .env is fixed', () => {
    ...
    // The bundle moves on: docker-compose.yml now needs a variable this
    // box's .env does not have -- exactly TOON_Network#152's shape of
    // regression, expressed the way this bundle expresses it.
    const brokenSha = addFixtureRequiredVar(origin.dir);

    // Run 2: the fetch brings back real work, the fast-forward succeeds, and
    // the apply fails on the missing variable.
    const second = autoApply(box);
    expect(second.status, 'an apply failure must fail the run').not.toBe(0);
    expect(
      headSha(box),
      'the fast-forward still happens; only the apply fails'
    ).toBe(brokenSha);
    expect(second.stderr).toContain('RELAY_FIXTURE_FLAG');
    expect(second.stderr).toMatch(/FAILED: 'docker compose/);
    ...
    expect(third.stderr).toContain('RELAY_FIXTURE_FLAG');
    ...
    // their own .env.
    writeEnv(box, { ...ENV, RELAY_FIXTURE_FLAG: 'ok' });
