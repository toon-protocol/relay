...
describe('SqliteEventStore', () => {
  ...
  describe('Error Handling', () => {
    it('should throw BlsBaseError with STORAGE_ERROR code on database errors', () => {
      store = new SqliteEventStore(':memory:');
      store.close();
      ...
      try {
        ...
        expect(error).toBeInstanceOf(BlsBaseError);
        expect((error as BlsBaseError).code).toBe('STORAGE_ERROR');
      }
    });
  });
});
