 ...
 * - EVM_CHAIN_ID: Chain identifier used in settlement config (default: evm:base:31337)
 * - PEER_EVM_ADDRESS: EVM address of the settlement peer
 * - M2M_TOKEN_ADDRESS: ERC-20 token contract address used for settlement
 * - TOKEN_NETWORK_REGISTRY: Token network registry contract address
 */
...
async function main(): Promise<void> {
  ...
  // -------------------------------------------------------------------------
  const chainId = process.env['EVM_CHAIN_ID'] || 'evm:base:31337';
  const settlementInfo: SettlementConfig = {
    supportedChains: [chainId],
    settlementAddresses: {
      [chainId]: process.env['PEER_EVM_ADDRESS'] || '',
    },
    preferredTokens: {
      [chainId]:
