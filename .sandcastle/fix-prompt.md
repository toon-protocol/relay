...
This is **relay** — a pnpm workspace (Node 22, pnpm 8.15.9). The checks that can go red:

- **CI / build** (`.github/workflows/ci.yml`), the gate run on EVERY PR: install with
  `pnpm install --frozen-lockfile`, then `pnpm -r build`, `pnpm typecheck`,
  `pnpm -r test --if-present`, plus lint/format against the frozen baseline
  (`pnpm lint`, `pnpm format:check`). Reproduce locally with the same commands, and run
  the BUILD before the TYPECHECK — typecheck resolves workspace `dist/` output, so an
...
3. Re-run what failed locally (`pnpm -r build`, `pnpm typecheck`, `pnpm lint`,
