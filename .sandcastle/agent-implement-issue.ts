...
//                             dead credential (toon-meta#248, connector#462) —
//                             see ./mint-app-token.ts. HOST ONLY: absent from
//                             PASSTHROUGH_KEYS in ./sandbox-secrets.ts, so it
//                             never enters the sandbox container. Optional —
...
// relay is a pnpm workspace: install with the committed lockfile so the sandbox
...
// (toon-meta#248, ported from connector#463 / connector#462)
// ---------------------------------------------------------------------------
...
try {
  ...
  // PUBLISH EARLY (toon-meta#248 / connector#462). The implementer has
  // committed; get those commits onto origin NOW rather than after the
  ...
  if (autoMerge && !blocking) {
    ...
    // run's total length is irrelevant (toon-meta#248). This also refreshes the
    // host's GH_TOKEN, which the `gh` calls below depend on.
