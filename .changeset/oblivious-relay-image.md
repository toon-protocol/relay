...
- the dependencies `@toon-protocol/connector`, `@toon-protocol/sdk`, `viem`,
  and the `@toon-protocol/bls` workspace dependency.

The `oblivious` mode flag is gone because it is now the only behavior. Identity
is derived with `nostr-tools` (NIP-06 for mnemonics); `NOSTR_SECRET_KEY` is
honored as an alias for `TOON_SECRET_KEY`. Ships as the
`ghcr.io/toon-protocol/relay:oblivious` image (the connector compose default;
tag retained for compatibility).
